      ...
      $.get("http://localhost:8282/insured/phone/" + phone)
        .done(function(response) {
          ...
          if (insuredData.status === "approved") {
            ...
            // Display the "Generate PDF" link
            var generatePdfLink = `
              <a href="#" class="generate-pdf-link" data-insured-id="${insuredData.id}">policy.pdf</a>
            `;
            $("#generatePdfLinkContainer").html(generatePdfLink);
...
$(document).on('click', '.generate-pdf-link', function(e) {
    ...
    $.ajax({
        url: 'http://localhost:8383/api/insured/' + insuredId + '/generate-pdf',
        type: 'GET',
        xhrFields: {
