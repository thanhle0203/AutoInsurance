...
public class InsuredController {
    ...
    public ResponseEntity<byte[]> generateInsurancePdf(@PathVariable Long insuredId) {
        ...
        if (insured != null && insured.getStatus().equals("approved")) {
            ...
            //if (pdfFile != null) {
                ...
                headers.setContentDisposition(ContentDisposition.builder("attachment").filename("policy.pdf").build());
    ...
    private byte[] generatePdf(Insured insured) {
        ...
        try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            ...
            document.open();
            document.add(new Paragraph("Policy ID: " + insured.getId()));
            
            document.add(new Paragraph("Customer Information"));
            document.add(new Paragraph("Name: " + insured.getName()));
            document.add(new Paragraph("Address: " + insured.getAddress().getStreet() + ", " + insured.getAddress().getCity() + ", " + insured.getAddress().getState() + " " + insured.getAddress().getZipCode()));
            document.add(new Paragraph("Phone: " + insured.getPhone()));
            document.add(new Paragraph("Email: " + insured.getEmail()));
            
            document.add(new Paragraph("Vehicle Information"));
            document.add(new Paragraph("Vehicle VIN: " + insured.getVehicle().getVin()));
            document.add(new Paragraph("Vehicle Make: " + insured.getVehicle().getMake()));
            document.add(new Paragraph("Vehicle Model: " + insured.getVehicle().getModel()));
            document.add(new Paragraph("Vehicle Year: " + insured.getVehicle().getYear()));
            
            document.add(new Paragraph("Auto Insurance Plan Information"));
            document.add(new Paragraph("Auto Plan Name: " + insured.getAutoInsurance().getAutoPlan().getName()));
            document.add(new Paragraph("Auto Plan Type: " + insured.getAutoInsurance().getAutoPlan().getType()));
            document.add(new Paragraph("Auto Plan Description: " + insured.getAutoInsurance().getAutoPlan().getDescription()));
            document.add(new Paragraph("Collison Deductible: " + insured.getAutoInsurance().getCollisionDeductible()));
            document.add(new Paragraph("Uninsured Motorist Deductible: " + insured.getAutoInsurance().getUninsuredMotoristDeductible()));
            document.add(new Paragraph("Total Price $: " + insured.getAutoInsurance().getTotalPrice() + "/year"));
